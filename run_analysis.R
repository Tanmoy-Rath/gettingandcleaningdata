...
TOTAL_PATH <- file.path(getwd(),"UCI HAR Dataset")
...
if(  !dir.exists(TOTAL_PATH)  ){
        ...
        (unzipped_file_list <- unzip("UCI HAR Dataset.zip",exdir="zip_directory"))
}else{
...
subject_train <- fread("zip_directory//UCI HAR Dataset//train//subject_train.txt", data.table = FALSE, na.strings=c("",NA))
X_train <- fread("zip_directory//UCI HAR Dataset//train//X_train.txt", data.table = FALSE, na.strings=c("",NA))
y_train <- fread("zip_directory//UCI HAR Dataset//train//y_train.txt", data.table = FALSE, na.strings=c("",NA))

subject_test <- fread("zip_directory//UCI HAR Dataset//test//subject_test.txt", data.table = FALSE, na.strings=c("",NA))
X_test <- fread("zip_directory//UCI HAR Dataset//test//X_test.txt", data.table = FALSE, na.strings=c("",NA))
y_test <- fread("zip_directory//UCI HAR Dataset//test//y_test.txt", data.table = FALSE, na.strings=c("",NA))
...
features <- fread("zip_directory//UCI HAR Dataset//features.txt", data.table = FALSE, na.strings=c("",NA))
...
# Quoted from features_info.txt:- meanFreq(): Weighted average of the frequency components to obtain a mean frequency
rm(required_columns)
...
activity_labels <- fread("zip_directory//UCI HAR Dataset//activity_labels.txt", data.table = FALSE, na.strings=c("",NA))
